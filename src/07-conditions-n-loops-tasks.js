...
function isInsideCircle(circle, point) {
  ...
  if (Math.sqrt(x ** 2 + y ** 2) < circle.radius) {
    return true;
  }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (a < b) {
    resultString += `${String(a)}, ${String(b)}`;
  } else {
    resultString += `${String(b)}, ${String(a)}`;
  }
  if (isEndIncluded) {
    resultString += ']';
  } else {
...
function isBracketsBalanced(str) {
  ...
  while (list.length !== 0) {
    ...
    if (opened.includes(list[i])) {
      i += 1;
    } else {
      lastOp = list[i];
      if (opened.indexOf(list[i - 1]) === closed.indexOf(lastOp)) {
        list.splice(i - 1, 2);
      } else {
        return false;
      }
...
function toNaryString(num, n) {
  ...
  let result = '';
  while (newN !== 0) {
    result += String(newN % n);
    newN = Math.floor(newN / n);
  }
